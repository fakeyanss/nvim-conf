...
plugin_key_mapping["toggleterm"] = function()
	bind.nvim_load_mapping({
		...
		["n|<leader>t-"] = map_cr([[execute v:count . "ToggleTerm direction=horizontal"]])
			:with_noremap()
			:with_silent()
		...
		["t|<leader>t-"] = map_cmd("<Esc><Cmd>ToggleTerm<CR>")
			...
			:with_silent()
			:with_desc("terminal: Toggle horizontal"),
		["n|<leader>t\\"] = map_cr([[execute v:count . "ToggleTerm direction=vertical"]])
			:with_noremap()
			:with_silent()
		...
		["t|<leader>tg"] = map_callback(function()
				toggle_lazygit()
			end)
			:with_noremap()
			:with_silent()
